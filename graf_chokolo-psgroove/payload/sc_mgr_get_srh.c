...
#define SC_MGR_GET_SRH_BASE					0x80000000133A0000ULL
#define SC_MGR_GET_SRH_OFFSET				0x133A0000ULL
#define SC_MGR_GET_SRH_PAGE_SIZE			12
#define SC_MGR_GET_SRH_SIZE					(1 << SC_MGR_GET_SRH_PAGE_SIZE)

static volatile u64 subject_id[2] = { 0x1070000002000001, 0x10700003FF000001 };

int sc_mgr_get_srh(void)
{
#define N(a)	(sizeof((a)) / sizeof((a)[0]))
	...
	struct ss_sc_mgr_get_srh *ss_sc_mgr_get_srh;
	int i, result;

	result = lv1_allocate_memory(SC_MGR_GET_SRH_SIZE, SC_MGR_GET_SRH_PAGE_SIZE,
		0, 0, &vuart_lpar_addr, &muid);
 	if (result != 0)
 		return result;

	MM_LOAD_BASE(msgbuf, SC_MGR_GET_SRH_OFFSET);

	result = mm_map_lpar_memory_region(0, MM_EA2VA((u64) msgbuf), vuart_lpar_addr,
		SC_MGR_GET_SRH_SIZE, SC_MGR_GET_SRH_PAGE_SIZE, 0, 0);
	if (result != 0)
		return result;

	memset(msgbuf, 0, SC_MGR_GET_SRH_SIZE);
	...
	dispmgr_header->response_size = sizeof(struct ss_header) +
		sizeof(struct ss_sc_mgr_get_srh);

	ss_header = (struct ss_header *) (dispmgr_header + 1);
	memset(ss_header, 0, sizeof(struct ss_header));
	ss_header->packet_id = 0x9001;
	ss_header->function_id = 0x9000;
	ss_header->laid = subject_id[0];
	ss_header->paid = subject_id[1];

	ss_sc_mgr_get_srh = (struct ss_sc_mgr_get_srh *) (ss_header + 1);
	memset(ss_sc_mgr_get_srh, 0, sizeof(struct ss_sc_mgr_get_srh));

	dispmgr_header->request_size += sizeof(struct ss_sc_mgr_get_srh);
