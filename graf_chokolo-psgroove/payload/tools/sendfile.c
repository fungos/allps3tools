...
{
	int c, n, i, j, proto, packet_size, pause_us, retransmit, use_vlan, file_size, num_packets, npacket;
	char *end;
	libnet_t *ln_ctx;
	...
	while ((c = getopt(argc, argv, "p:s:w:r:v")) != -1)
	{
		switch (c)
		{
			case 'p':
	...
	/* create libnet context for sending packets */
	...
	/* create pcap filter for incoming ACKs */
	...
	sprintf(pcap_fp_str, "%sether proto 0x%04x and ether src %02x:%02x:%02x:%02x:%02x:%02x",
		(use_vlan == 1) ? "vlan and " : "",
	...
	for (i = optind + 2; i < argc; i++)
	{
		...
		npacket = 0;
		eth_hlen = (use_vlan == 1) ? VLAN_ETH_HLEN : ETH_HLEN;
		...
		while ((n = fread(buf + PKT_HDR_SIZE, 1, packet_size, fp)) > 0)
		{
			...
			for (j = 0; j < retransmit + 1; j++)
			{
				...
				if (pcap_hdr.len < (eth_hlen + PKT_HDR_SIZE))
				{
					fprintf(stderr, "invalid ack for packet #%d\n", npacket);
					goto next;
				}

				pkt_hdr = (struct pkt_hdr *) ((u_int8_t *) vlan_eth_hdr + eth_hlen);
...
static void usage(const char *progname)
{
	fprintf(stderr, "usage: %s [-p <proto>] [-s <packet size>] [-w <pause us>] "
		"[-r <max retransmits>] [-v] <interface> <dest mac address> <filename> [<filename>]\n", progname);
	exit(1);
}
