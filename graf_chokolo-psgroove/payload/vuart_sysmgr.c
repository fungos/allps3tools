...
{
	u64 lpar_addr, muid, nread, nwritten;
	void *msgbuf;
	...
	result = mm_map_lpar_memory_region(0, MM_EA2VA((u64) msgbuf), lpar_addr,
		VUART_SYSMGR_SIZE, VUART_SYSMGR_PAGE_SIZE, 0, 0);
 	if (result != 0)
 		return result;
	...
	result = vuart_wait_for_rx_data(SYSMGR_VUART_PORT);
	if (result < 0)
		return result;
