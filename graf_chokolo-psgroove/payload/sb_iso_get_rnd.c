...
static volatile u8 __attribute__ ((aligned ((128)))) spu_args[0x80];

int sb_iso_get_rnd(void)
{
	...
	struct sb_iso_get_rnd *sb_iso_get_rnd;
		...
		if (intr_status & 0x1)
		{
			...
			if (spu_mbox_stat_intr_out_mbox_count(spu_problem) != 0)
			{
				...
				else if (spu_out_intr_mbox_value == 0x80)
				{
					...
					sb_iso_header->cmd = 0x2005UL;
					sb_iso_header->cmd_size = sizeof(struct sb_iso_get_rnd);

					sb_iso_get_rnd = (struct sb_iso_get_rnd *) (sb_iso_header + 1);
					memset(sb_iso_get_rnd, 0, sizeof(struct sb_iso_get_rnd));
					sb_iso_get_rnd->field0 = SB_ISO_REPO_NODE_BUS_1_ID_V2;
