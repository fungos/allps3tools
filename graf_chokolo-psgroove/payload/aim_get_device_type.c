...
#define AIM_GET_DEVICE_TYPE_BASE				0x80000000133A0000ULL
#define AIM_GET_DEVICE_TYPE_OFFSET				0x133A0000ULL
#define AIM_GET_DEVICE_TYPE_PAGE_SIZE			12
#define AIM_GET_DEVICE_TYPE_SIZE				(1 << AIM_GET_DEVICE_TYPE_PAGE_SIZE)

static volatile u64 subject_id[2] = { 0x1070000002000001, 0x10700003FF000001 };

int aim_get_device_type(void)
{
#define N(a)	(sizeof((a)) / sizeof((a)[0]))
	...
	struct ss_aim_get_device_type *ss_aim_get_device_type;
	int result;

	result = lv1_allocate_memory(AIM_GET_DEVICE_TYPE_SIZE, AIM_GET_DEVICE_TYPE_PAGE_SIZE,
		0, 0, &vuart_lpar_addr, &muid);
 	if (result != 0)
 		return result;

	MM_LOAD_BASE(msgbuf, AIM_GET_DEVICE_TYPE_OFFSET);

	result = mm_map_lpar_memory_region(0, MM_EA2VA((u64) msgbuf), vuart_lpar_addr,
		AIM_GET_DEVICE_TYPE_SIZE, AIM_GET_DEVICE_TYPE_PAGE_SIZE, 0, 0);
	if (result != 0)
		return result;

	memset(msgbuf, 0, AIM_GET_DEVICE_TYPE_SIZE);
	...
	dispmgr_header->response_size = sizeof(struct ss_header) +
			sizeof(struct ss_aim_get_device_type);

	ss_header = (struct ss_header *) (dispmgr_header + 1);
	memset(ss_header, 0, sizeof(struct ss_header));
	ss_header->packet_id = 0x19002;
	ss_header->function_id = 0x19000;
	ss_header->laid = subject_id[0];
	ss_header->paid = subject_id[1];

	ss_aim_get_device_type = (struct ss_aim_get_device_type *) (ss_header + 1);
	memset(ss_aim_get_device_type, 0, sizeof(struct ss_aim_get_device_type));

	dispmgr_header->request_size += sizeof(struct ss_aim_get_device_type);
	...
	result = lv1_read_virtual_uart(DISPMGR_VUART_PORT, vuart_lpar_addr,
		AIM_GET_DEVICE_TYPE_SIZE, &nread);
	if (result < 0)
		return result;
