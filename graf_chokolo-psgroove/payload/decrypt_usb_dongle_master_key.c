...
static volatile u8 encrypted_usb_dongle_master_key[64] =
{
...
static volatile u64 subject_id[2] = { 0x1070000001000001, 0x1070000045000001 };
...
int decrypt_usb_dongle_master_key(void)
{
	...
	result = mm_map_lpar_memory_region(0, MM_EA2VA((u64) msgbuf),  vuart_lpar_addr,
		DECRYPT_USB_DONGLE_MASTER_KEY_SIZE, DECRYPT_USB_DONGLE_MASTER_KEY_PAGE_SIZE, 0, 0);
	if (result != 0)
		return result;
