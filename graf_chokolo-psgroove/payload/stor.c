...
int stor_init(u64 dev_id)
{
	...
	result = mm_map_lpar_memory_region(0, MM_EA2VA(STOR_DMA_BASE), dma_lpar_addr,
		STOR_DMA_SIZE, STOR_DMA_PAGE_SIZE, 0, 0);
	if (result != 0)
		return result;
...
int stor_write(u64 region_id, u64 start_sector, u64 num_sectors, u64 flags, void *data)
{
	...
	memcpy(buf, data, stor_var.block_size * num_sectors);

	result = lv1_storage_write(stor_var.dev_id, region_id, start_sector, num_sectors,
		flags, stor_var.dma_lpar_addr, &tag);
	if (result != 0)
		return result;
