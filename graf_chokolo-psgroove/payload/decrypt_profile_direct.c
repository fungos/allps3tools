...
int decrypt_profile_direct(void)
{
	...
	u8 mfc_cmd_tag;
 	int metldr_size, isoldr_size, rvkprg_size, spu_module_size, profile_size, i, result;
	...
	profile = spu_module + SPU_MODULE_SIZE;

	profile_size = gelic_recv_data(profile, PROFILE_SIZE);
	if (profile_size <= 0)
		return profile_size;
		...
		if (intr_status & 0x1)
		{
			...
			if (spu_mbox_stat_intr_out_mbox_count(spu_problem) != 0)
			{
				...
				if (spu_out_intr_mbox_value == 1)
				{
					...
					ldr_args.spu_module_arg1_size = profile_size;
					ldr_args.field48 = 3;
	...
	result = gelic_xmit_data(gelic_bcast_mac_addr, 0xBEEF, profile, profile_size);
	if (result != 0)
		return result;
