...
static volatile u8 spu_args_field60[16] =
{
...
int decrypt_npdrm(void)
{
	...
	for (i = 0; i < elf64_hdr->e_phnum; i++)
	{
		...
		spu_args.field30 = 0x2010004ULL;
		spu_args.field40[0] = 1ULL << 62;
		spu_args.field40[3] = 2;

		memcpy(spu_args.field60, spu_args_field60, sizeof(spu_args_field60)); 

		memcpy(spu_args.field70, self + (*(u64 *) (self + 0x58)) + 0x80, 0x80);
