...
#define FIRMWARE_SIZE				0x1000000
...
static volatile u8 __attribute__ ((aligned ((128)))) spu_args[0x1100];

int sc_iso_sc_binary_patch(void)
{
	u64 priv2_addr, problem_phys, local_store_phys, unused, shadow_addr, spe_id, intr_status;
	u8 *metldr, *isoldr, *rvkprg, *spu_module, *firmware;
	...
	u8 mfc_cmd_tag;
	struct sc_iso_debug_buffer *sc_iso_debug_buffer;
	struct sc_iso_header *sc_iso_header;
	struct sc_iso_sc_binary_patch *sc_iso_sc_binary_patch;
 	int metldr_size, isoldr_size, rvkprg_size, spu_module_size, firmware_size, i, result;
	...
	firmware = spu_module + SPU_MODULE_SIZE;

	firmware_size = gelic_recv_data(firmware, FIRMWARE_SIZE);
	if (firmware_size <= 0)
		...
		if (intr_status & 0x1)
		{
			...
			if (spu_mbox_stat_intr_out_mbox_count(spu_problem) != 0)
			{
				...
				if (spu_out_intr_mbox_value == 1)
				{
					...
					sc_iso_debug_buffer = (struct sc_iso_debug_buffer *) spu_args;
					sc_iso_debug_buffer->buf = debug_buffer;
					sc_iso_debug_buffer->buf_size = sizeof(debug_buffer);
				...
				{
					sc_iso_header = (struct sc_iso_header *) spu_args;
					memset(sc_iso_header, 0, sizeof(struct sc_iso_header));
					sc_iso_header->seqno = 1;
					sc_iso_header->mbmsg = 1;
					sc_iso_header->cmd = 0x80000013UL;
					sc_iso_header->cmd_size = sizeof(struct sc_iso_sc_binary_patch) + firmware_size;

					sc_iso_sc_binary_patch = (struct sc_iso_sc_binary_patch *) (sc_iso_header + 1);
					memset(sc_iso_sc_binary_patch, 0, sizeof(struct sc_iso_sc_binary_patch));
					sc_iso_sc_binary_patch->data_size = firmware_size;
					memcpy(sc_iso_sc_binary_patch->data, firmware, firmware_size);
